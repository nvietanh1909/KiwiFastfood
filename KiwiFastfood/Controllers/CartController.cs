    ...
    public class CartController : Controller
    {
        ...
        public async Task<ActionResult> Remove(string productId)
        {
            ...
            try
            {
                string token = Session["UserToken"].ToString();
                _cartService.SetToken(token);
                var response = await _cartService.RemoveItemAsync(productId);
                dynamic result = JsonConvert.DeserializeObject(response);

                if (result?.success != true)
                {
                    TempData["ErrorMessage"] = "Không thể xóa sản phẩm khỏi giỏ hàng: " + (result?.error ?? "Lỗi không xác định.");
                }
                else
                {
            ...
            catch (Exception ex)
            {
                ...
                TempData["ErrorMessage"] = "Không thể xóa sản phẩm khỏi giỏ hàng: " + ex.Message;
                return RedirectToAction("Cart");
            }

        }
        [HttpPost]
        public async Task<ActionResult> Clear(string productId)
        {
            if (!_isLogin)
            {
                return RedirectToAction("Login", "User");
